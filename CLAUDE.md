...
All 18 game ids are in [registry.ts](packages/shared/src/games/registry.ts); server modules, host and controller components all live under matching `<id>/` folders (Lokalni megdan is the one exception — its id is `osvajanje` and its folders are `bitka/`). Non-obvious things worth knowing:
