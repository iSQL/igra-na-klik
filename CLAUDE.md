...
- **Board model**: a *puzzle* is 4 columns (letters A/B/C/D by index) × 4 fields + a `finalSolution`. Each `AsocijacijeField` has a `word` (the association revealed) and, for kviz mode, an optional `question` + `wrongOptions[]` (the correct answer is always `word`, merged into the shuffled options server-side). A puzzle is **kviz-capable** iff every field has a usable question (`isKvizCapablePuzzle`). Number of boards per game rides the generic `roundCount` / `GAME_ROUND_CONFIG` knob (default 1).
...
- **Packs**: `asocijacije-packs/<id>.json` (override `ASOCIJACIJE_PACKS_DIR`), manifest `{ name?, description?, puzzles: [...] }`; **manifests carry answers**, so `GET /api/asocijacije-packs` returns summaries only (puzzleCount + kvizPuzzleCount + visibleInGame), with the built-in bank prepended as the pseudo-pack `ASOCIJACIJE_BANK_PACK_ID` (`'__bank__'`). Selection is single-pack (default bank) via the game-select "Slagalice" picker; the server resolves the chosen pack by id (`asocijacije-pack-resolver.ts`, sync at game start). In kviz mode only kviz-capable puzzles are used (falls back to the whole pool if none). Admin editor: the "Asocijacije" view in the `/admin` SPA (kind `asoc` in [admin-app.ts](packages/server/src/admin/admin-app.ts)) — a structured 4×4 form with a local working copy PUT whole via `content-admin.ts`.
