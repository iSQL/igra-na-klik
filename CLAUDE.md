...
- **Roles are data** in [packages/shared/src/games/gluvo-doba-roles.ts](packages/shared/src/games/gluvo-doba-roles.ts): `GLUVO_DOBA_ROLES` (name/team/description/nightPrompt/`nightActionType`/hint group), `compositionFor(n, opts)` builds the deck per player-count band (6–8 / 9–12 / 13–15; always exactly 2 wolves, the dark side scales through specialists — Todorac blocker, Drekavac false-scan, Bauk vote-mute), plus optional neutrals (Lesnik survivor 9–12, Morana solo killer 13–15) and Vila via host config toggles. The Vračara's investigation returns deliberately ambiguous `GLUVO_DOBA_HINT_GROUPS` mixing teams. Add a new role by extending the tables + the resolution pipeline, not with if-branches.
