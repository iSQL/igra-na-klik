...
The same `ADMIN_TOKEN` gate also enables four JSON-only content editors: `/admin/kviz` (question packs), `/admin/ko-sam-ja` (personal-question packs, all four shapes), `/admin/tajni-agenti` (word packs, one word per line) and `/admin/gluvo-doba` (Gluvo doba role packs — wolf count + a checkbox grid of enabled roles; see the Gluvo doba section). The gluvo-doba route lives under `/api/admin/gluvo-doba-packs` and re-validates through `parseGluvoDobaPack` on every write (strict — invalid packs are stored but flagged `visibleInGame:false`). Bare `/admin` redirects to `/admin/geo`; every admin page carries a shared nav. Shared plumbing lives in [packages/server/src/admin/admin-common.ts](packages/server/src/admin/admin-common.ts) (requireAdmin, slugify, atomic JSON writes) and [admin-shell.ts](packages/server/src/admin/admin-shell.ts) (page shell + token gate + `Admin` JS runtime — page scripts must still avoid backticks and dollar-brace); the API router is [content-admin.ts](packages/server/src/admin/content-admin.ts) with per-type routes under `/api/admin/{quiz-packs,ko-sam-ja-packs,tajni-agenti-packs}`. Unlike geo there are no images, so editing is whole-file `PUT` (list/create/replace/delete). Key policy: **reads are lax, writes create drafts** — quiz/ko-sam-ja PUTs re-validate through the shared strict parsers (the form builds one valid question at a time), while tajni-agenti packs (< 25 words) accept drafts that fail the strict in-game check; such files stay editable but invisible in the game, and the API response carries `visibleInGame` + `error` so the page can say why.

### Pogodi broj (guess-a-number, formerly "Pogodi godinu")

A slider guessing game generalized from years to **any numeric quantity** — the closest guess to the true value scores most (exponential falloff). The internal game id and directory stay `pogodi-godinu`; only the display name/description (registry + i18n `game.pogodi-godinu.*`) changed to **"Pogodi broj"**. Each question ([`PogodiBrojQuestion`](packages/shared/src/types/pogodi-godinu.ts)) carries its own `min`/`max` slider range, plus optional `step`, `unit` ("din"/"kg"/"km"/"god."/"slova"…), `valueType` (`'duration'` renders mm:ss), `emoji`, and `imageFile` (packs only). "Counting" questions ("koliko slova A…", "koliko pasa nosi plavu kapu") are just numeric questions with a small range — no special code. `formatPogodiBrojValue(value, unit, valueType)` in [pogodi-broj-import.ts](packages/shared/src/games/pogodi-broj-import.ts) is the shared host+controller formatter.

- **Scoring** is range-normalized: `pointsForDistance(distance, span)` with `span = max - min` and `K = 6` ([scoring.ts](packages/server/src/game/games/pogodi-godinu/scoring.ts)) — preserves the old year-only feel while staying fair for any magnitude (same idea as geo scaling decay to map size). The guess action is `broj:guess { value }`, clamped + step-snapped server-side.
- **Question packs** mirror geo: `pogodi-broj-packs/<id>.json` (validated by `parsePogodiBrojImport`) + `<id>/` image folder served at `/pogodi-images/<id>/<file>`. `GET /api/pogodi-broj-packs` lists summaries; [pack-resolver.ts](packages/server/src/game/games/pogodi-godinu/pack-resolver.ts) resolves the chosen pack (`pogodiBrojPackId` in `host:start-game`, async-loaded in `onStart` like geo — falls back to the built-in bank if empty/missing). No pack → built-in mixed bank `POGODI_BROJ_QUESTIONS` ([pogodi-godinu-events.ts](packages/shared/src/games/pogodi-godinu-events.ts)), which now mixes migrated year questions + prices/weights/distances/durations/counting.
- **Admin editor** `/admin/pogodi-broj` ([pogodi-broj-admin.ts](packages/server/src/admin/pogodi-broj-admin.ts) + [pogodi-broj-editor-page.ts](packages/server/src/admin/pogodi-broj-editor-page.ts)) is modeled on geo-admin (base64 image upload → `<uuid>.jpg`, atomic writes, granular per-question POST/PATCH/DELETE with the image inline). Drafts (0 questions) are allowed via `allowEmpty`; the API response carries `visibleInGame` + `error`. Dir override: `POGODI_BROJ_PACKS_DIR`.
