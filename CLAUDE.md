...
- **Game gating**: only games with `supportsHostless: true` in `GAME_DEFINITIONS` (currently Kviz, Crtaj i pogodi, Lažov and Ko sam ja) can start in a hostless room. Validated server-side in `GameManager.startGame`; the controller's game-select greys the rest out with "Zahteva TV ekran". To make another game hostless-capable, ensure its controller UI shows everything the TV would (the shared data already arrives via the stripped `game:state-update` broadcast), then flip the flag.
