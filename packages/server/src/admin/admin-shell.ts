...
.admin-nav a{padding:0.35rem 0.85rem;border-radius:9px;font-size:0.8rem;font-weight:800;text-decoration:none;color:var(--muted);border:1px solid var(--line)}
.admin-nav a:hover{color:var(--ink);border-color:var(--line2)}
.admin-nav a.active{background:var(--grad);color:#fff;border-color:transparent}
`;
...
:root{
  ...
  --green:#2fe08a;--red:#ff4d5e;--blue:#3b82f6;
...
input:focus,textarea:focus,select:focus{outline:none;border-color:var(--cyan);box-shadow:0 0 0 3px rgba(34,222,230,.15)}
...
.badge-ok{background:rgba(47,224,138,.14);color:var(--green)}
...
.tag-nsfw{background:rgba(255,77,94,.16);color:var(--red)}
.tag-cyan{background:rgba(34,222,230,.14);color:var(--cyan)}
${ADMIN_NAV_CSS}
${opts.extraCss ?? ''}
</style>
</head>
